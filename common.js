...
.wm-item{
  ...
  white-space:nowrap; user-select:none; animation:wm-sway var(--dur,12s) ease-in-out infinite alternate;
}
@keyframes wm-sway{
...
.topbar{
  ...
  grid-template-columns: auto 1fr auto; /* 左品牌｜中導覽｜右按鈕或佔位 */
  align-items:center;
  padding:16px 32px;
...
.brand img{ height:60px; width:auto; transition:transform .3s ease; }
...
.nav-links{
  ...
  gap:40px; white-space:nowrap; overflow-x:auto; -webkit-overflow-scrolling:touch;
}
.nav-links a{
  text-decoration:none; color:#333; font-weight:600; transition:color .2s, transform .2s; white-space:nowrap;
}
.nav-links a:hover{ color:#ff9800; transform:translateY(-1px); }
.nav-links a.active{ color:#ff9800; border-bottom:2px solid #ff9800; padding-bottom:2px; }

.actions{ grid-column:3; justify-self:end; display:flex; gap:10px; white-space:nowrap; }
.actions-ghost{ min-width:320px; height:1px; }
@media (max-width: 860px){
  .topbar{ padding:14px 16px; }
  .nav-links{ gap:20px; }
  .actions-ghost{ min-width:0; }
}
.divider{ height:10px; }
...
.grid-3 { display:grid; grid-template-columns:1.15fr 0.95fr 1.15fr; gap:0px; }
.card{
  background:var(--card);
...
/* =====================
   元件
===================== */
.btn{ border:none; cursor:pointer; padding:8px 14px; border-radius:10px; font-weight:700; color:#fff; background:var(--green); }
...
.kv .kv-row{ display:flex; justify-content:space-between; align-items:center; padding:6px 0; font-weight:600; }

/* =====================
   基本資料區
===================== */
.form-row{ display:flex; align-items:center; gap:10px; margin:12px 0; font-size:15px; }
.form-row > label{ flex:0 0 90px; white-space:nowrap; }
.form-row .chips,.form-row .cms{ flex:1 1 auto; display:flex; gap:8px; flex-wrap:nowrap; min-width:0; }
.form-row input[type="number"],.form-row input[type="text"]{ height:34px; font-size:15px; border-radius:6px; width:100px; padding:2px 6px; }

.chips{ display:flex; gap:8px; }
.chip{
  display:inline-flex; align-items:center; gap:6px;
  padding:5px 10px; border:1px solid var(--line); border-radius:999px; background:#fff; font-size:14px; cursor:pointer; user-select:none;
}
...
/* =====================
   服務表格（共用）
===================== */
.service-card h3 { margin:24px 0 10px 2px; font-size:16px; font-weight:800; color:#6b6544; }
#tables { width:100%; }
#tables table {
  width:100%; border-collapse:separate; border-spacing:0; background:#fff; border:1px solid var(--line); border-radius:12px; overflow:hidden;
}
#tables thead th {
  position:sticky; top:0; z-index:5; background:#f6f1d9; color:#584f2a;
  padding:12px 18px; border-bottom:1px solid var(--line); font-weight:800;
}
#tables th:first-child, #tables td:first-child { text-align:left; padding-left:18px; }
#tables th:nth-child(2), #tables th:nth-child(3), #tables th:nth-child(4), #tables th:nth-child(5), #tables th:nth-child(6){
  width:140px; text-align:center;
}
#tables td:nth-child(2), #tables td:nth-child(3), #tables td:nth-child(4), #tables td:nth-child(5), #tables td:nth-child(6){
  text-align:center; vertical-align:middle;
}
#tables tbody td { padding:12px 18px; border-top:1px solid var(--line); vertical-align:middle; }
...
/* 手機板表格：單行＋橫向滑動 */
#tables { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; display: block; }
#tables table { width: max-content; min-width: 100%; border-collapse: collapse; table-layout: auto; }
#tables th, #tables td { white-space: nowrap !important; text-align: right; padding: 6px 10px; }
#tables th:first-child, #tables td:first-child { text-align: left; }
#tables .cell-price { width: 80px; text-align: right; }
